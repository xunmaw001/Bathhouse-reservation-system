<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="澡堂分类" prop="zaotangfenlei">
          <el-input v-model="ruleForm.zaotangfenlei" 
              placeholder="澡堂分类" clearable  :readonly="ro.zaotangfenlei"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  data() {
	...
	zaotangfenlei : false,
      ...
      rules: {
          zaotangfenlei: [
                { required: true, message: '澡堂分类不能为空', trigger: 'blur' },
          ],
      }
    };
  },
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zaotangfenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zaotangfenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
